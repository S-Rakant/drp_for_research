...
class EpisodeRunner:
    ...
    def run(self, test_mode=False):
        ...
        while not terminated:
            ...
            print(f'actions = {actions[0]}') #actions = tensor([16, 13,  3,  6])
    ...
    def process_value(k, cur_value, env_value):
        if isinstance(env_value, list):
            # リストの場合はリスト内の各要素を加算
            if isinstance(cur_value, list):
                # cur_value がリストの場合、対応する要素を加算
                return [float(x) + float(y) for x, y in zip(cur_value, env_value)]
            else:
                # cur_value がリストでない場合、env_value のリストの各要素をそのまま加算
                return [float(x) for x in env_value]
        elif isinstance(env_value, np.float64):
            # numpy.float64 を float に変換
            return float(env_value) + float(cur_value)
        else:
            # その他の型はそのまま加算
            return env_value + cur_value
