...
class DrpEnv(gym.Env):
	...
	def _get_avail_agent_actions(self, agent_id, n_actions):
		print(f'agent_id = {agent_id}, n_actions = {n_actions}, current_goal = {self.current_goal}')
	...
	def step(self, joint_action):
		'''安全制約
		相手が隣接ノード上にいるとき:
			自身のactionが相手の現在ノード+相手のactionが自身の現在ノード --> bad_actionに相手の現在ノードを追加
		相手がエッジ上にいるとき:
			進入してしまうノードをbad_actionに追加

  		'''
		print(f'step')
		print(f'joint_action = {joint_action}')
		print(f'pos_list = {self.get_pos_list()}')
		...
		restrict = [False for _ in range(self.agent_num)]
		...
		for age in range(self.agent_num):
			adjacent_node = self.get_avail_agent_actions(age, self.n_actions)[1]
			...
			if restrict[age]:
				...
				candidate = [int(item) for item in np.arange(self.n_actions) if item not in bad_action]
		...
		#安全制約に引っかかったagentにペナルティ
		for age, restricted in enumerate(restrict):
			if restricted:
				ri_array[age] += -50
