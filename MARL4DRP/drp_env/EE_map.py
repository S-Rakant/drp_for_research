...
class MapMake():
	...
	def get_avail_action_fun(self, obs_i, current_start, current_goal, goal_i):
		print(f'obs_i = {obs_i}, current_goal = {current_goal}')
		...
		action_set = []
		print(f'start = {self.start_ori_array}')
		print(f'goal = {self.goal_array}')
		print(f'pos = {self.pos}')
		print(f'pos.values()[0] = {list(self.pos.values())}')
		print(f"[obs_i[0],obs_i[1]] = {([float(obs_i[0]),float(obs_i[1])])}")
		# if str([obs_i[0],obs_i[1]]) in [str(ele) for ele in self.pos.values()]: #s=(0.0, 5.0)
		if [float(obs_i[0]),float(obs_i[1])] in list(self.pos.values()):
			print("it currently at node")
			# node = [k for k, v in self.pos.items() if str(v) == str([obs_i[0],obs_i[1]])][0]  #node 0
			node = [k for k, i in enumerate(list(self.pos.values())) if [float(obs_i[0]),float(obs_i[1])] == i][0]
			...
			print(f'current_start = {current_start}, current_goal = {current_goal}')
